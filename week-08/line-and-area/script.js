...
function generateDummyData(n){
	...
	return Array.from({length:n}).map(function(d,i){
		return {
			index: i,
			value: Math.sin((offset + i/n) * Math.PI * 1) * 2 + 10 + Math.random() * 1.5 //what kind of numbers is this returning to us?
		}
	})
}
...
function drawChart1(lineData){
	...
	const extentX = d3.extent(lineData, function(d){return d.index}); //finds max and min index value.
	const extentY = d3.extent(lineData, function(d){return d.value}); // finds max and min value value
	const scaleX = d3.scaleLinear().domain(extentX).range([0, w]); //takes the existing domain and sets it to a range of 0-w
	const scaleY = d3.scaleLinear().domain(extentY).range([h, 0]);// takes the existing domain and sets it to a range of h-0.
	...
	const area = d3.area() //takes in array and spits out string description of all the objects (coordinates)
		.x(function(d){ return scaleX(d.index) })
		.y1(function(d){ return scaleY(d.value) })
		.y0(h)//area needs not only the top line but also the bottom hight. So h = total height.
		//feel free to explore these additional parameters below by commenting / uncommenting each line
		.curve(d3.curveBasis)
		//.curve(d3.curveNatural)
		//.curve(d3.curveStep)
	...
	const plot1 = d3.select('#chart-1')
		...
		.attr('height', H)
		.append('g')
		.attr('transform', `translate(${margin.l}, ${margin.t})`);//need to remember what the `translate $ yada stuff means.
	...
	//Finally, draw the axes    *** Where did the axis lables come from? I don't see that code.
